...
impl super::VikunjaClient {
    pub async fn find_or_get_project_id(&self, app: &mut crate::tui::app::App, project_name: &str) -> reqwest::Result<Option<i64>> {
        let url = format!("{}/api/v1/projects", self.base_url);
        let normalized_input = project_name.trim().to_ascii_lowercase();
        utils::debug_log(app, &format!("Looking for project: '{}' (normalized: '{}')", project_name, normalized_input));
        ...
        utils::debug_log(app, &format!("Raw project list response: {}", text));
        let projects: Vec<VikunjaProject> = match serde_json::from_str(&text) {
            Ok(projects) => projects,
            Err(e) => {
                utils::debug_log(app, &format!("Failed to decode project list: {}", e));
                return Ok(None);
            }
        };
        utils::debug_log(app, &format!("Available projects: {:?}", projects.iter().map(|p| format!("{} (id={})", p.title, p.id)).collect::<Vec<_>>()));
        Ok(projects.iter()
            .filter(|p| p.id > 0)
